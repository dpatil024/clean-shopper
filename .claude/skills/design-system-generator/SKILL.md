...
Also check whether `/docs/design.md`, `/docs/design-tokens.md`, or `/docs/component-spec.md` already exist. If any do, this is a **regeneration**, not a fresh build — read them first, and treat the interview as a chance to confirm or revise the existing direction rather than starting from zero. Say so explicitly before beginning the interview, and note which existing tokens/components would be affected by a change in direction.
...
Create a file called `design-system-visual.html` inside `/docs/design/` (create that directory if it doesn't exist). This file is for the designer to open in a browser and review with their eyes. It must show every token visually: color swatches with hex values and usage labels, the full type scale rendered at each size, the spacing scale as visual blocks, border radius examples, and shadow examples.
...
Treat this file as a static snapshot, not a live view — say so in the file itself, since it will not update automatically if the tokens in code change later.

### Output 2: Markdown Specification File

Create a file called `design.md` inside the `/docs/` directory. If the `/docs/` directory does not exist, create it. This file is what Claude Code reads. It must be precise, structured, and complete.

If `/docs/design.md` already exists (a regeneration — see Step 1), replace its contents with the new spec, but preserve any "Open decisions" or historical/decision-log content from the old version that's still relevant, rather than silently discarding it.
...
Before presenting either output as done, run `/a11y-check` against the generated color system — specifically every semantic-color-on-neutral-background text pairing (success/warning/error text on neutral-50/100) at the sizes specified in the usage rules. Fix any failures by adjusting the generated hex values before finalizing, and note in the "Brand Direction" section if any color was adjusted from the designer's literal input to clear a contrast threshold.
...
```
- Design system: See /docs/design.md — follow these visual specifications for all UI work. Do not hardcode colors, font sizes, spacing, or shadows.
```

If a References section does not exist in CLAUDE.md, create one. If a line already references `/docs/design.md` or `/docs/design-tokens.md`, update that line in place rather than adding a duplicate.
